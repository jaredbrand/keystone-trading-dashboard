...
export default function LiveTradingDashboard() {
  ...
  const SHEET_ID = '1cpNfZD3TwDyyIg3ZpNS-SbtiIhi9GDtjiLv-3GCm_rI';
  ...
  const fetchSheetData = async () => {
    ...
    try {
      ...
      const summaryObj = {
        ...
        startingBankroll: parseFloat(summaryRows[1]?.[7]?.replace(/[$,]/g, '')) || 0, // H2 - Starting Bankroll (index 7, column H)
        duration: parseInt(summaryRows[1]?.[8]) || 0, // I2 - Duration (index 8, column I)
        wlWeeks: summaryRows[1]?.[9] || '', // J2 - W/L Weeks (index 9, column J)
        currentBankroll: parseFloat(summaryRows[1]?.[10]?.replace(/[$,]/g, '')) || 0, // K2 - Current Bankroll (index 10, column K)
      ...
      <main style={{ maxWidth: '1800px', margin: '0 auto', padding: '32px' }}>
        ...
        {activeView === 'summary' && summaryData && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px', marginTop: '32px' }}>
            ...
            <div className="stat-card" style={{ padding: '24px', borderRadius: '12px' }}>
              <div style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '8px', fontFamily: 'Inter, sans-serif', fontWeight: '500' }}>STARTING BANKROLL</div>
              <div style={{ fontSize: '28px', fontWeight: '800', color: '#D4AF37', fontFamily: 'Inter, sans-serif' }}>${summaryData.startingBankroll.toLocaleString()}</div>
            ...
            <div className="stat-card" style={{ padding: '24px', borderRadius: '12px' }}>
              <div style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '8px', fontFamily: 'Inter, sans-serif', fontWeight: '500' }}>CURRENT BANKROLL</div>
              <div style={{ fontSize: '28px', fontWeight: '800', color: '#10b981', fontFamily: 'Inter, sans-serif' }}>${summaryData.currentBankroll.toLocaleString()}</div>
              <div style={{ fontSize: '11px', color: '#64748b', marginTop: '8px', fontFamily: 'JetBrains Mono, monospace' }}>
                {summaryData.currentBankroll >= summaryData.startingBankroll ? '📈 GROWING' : '📉 BELOW START'}
              </div>
            </div>
            <div className="stat-card" style={{ padding: '24px', borderRadius: '12px' }}>
